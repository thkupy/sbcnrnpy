...
if __name__ == '__main__':
    ...
    if os.path.isfile('./results/Figure8a.npy'):
        print("Data for Figure8a found... loading!")
        output = np.load('./results/Figure8a.npy', allow_pickle=True)
        P = np.load('./results/Figure8a_P.npy', allow_pickle=True)
        P = P.tolist()
    else:
        ...
        np.save('./results/Figure8a.npy',output, allow_pickle=True)
        np.save('./results/Figure8a_P.npy',P, allow_pickle=True)

    ###Plot results
    fhandle=plotres(output=output,P=P,x=np.unique(P['D']),y=np.unique(P['L']),
            xlabs=r'Diameter $(\mu m)$',ylabs=u'Length $(\mu m)$')
    pp = PdfPages('./Figure8/Figure8a.pdf')
    pp.savefig()
    pp.close()
