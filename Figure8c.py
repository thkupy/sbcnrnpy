...
if __name__ == '__main__':
    ...
    if os.path.isfile('./results/Figure8c.npy'):
        print("Data for Figure8c found... loading!")
        output = np.load('./results/Figure8c.npy', allow_pickle=True)
        P = np.load('./results/Figure8c_P.npy', allow_pickle=True)
        P = P.tolist()
    else:
        ...
        np.save('./results/Figure8c.npy',output, allow_pickle=True)
        np.save('./results/Figure8c_P.npy',P, allow_pickle=True)

    ###Plot results
    fhandle=plotres(output=output,P=P,x=np.unique(P['cf'])/1000.0,y=np.unique(P['gKLT_d'])*1000.0,
            xlabs=r'$CF (kHz)$',ylabs=r'$g_{KLTdend} (nS)$')
    pp = PdfPages('./Figure8/Figure8c.pdf')
    pp.savefig()
    pp.close()
