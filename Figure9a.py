...
if __name__ == '__main__':
    ...
    ###Plot results
    fhandle=plotres(output=output,P=P,x=np.unique(P['cf'])/1000.0,y=np.unique(P['freq'])/1000.0,
            xlabs=r'$CF (kHz)$',ylabs=r'$Frequency (kHz)$')
    pp = PdfPages('./Figure9/Figure9a.pdf')
    pp.savefig()
    pp.close()
