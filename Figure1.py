...
"""
Created 2019-07-11
Creates raw figures that will be figure 1 of the Koert & Kuenzel Paper.
...
def runmain():
    ...
    N=32
    freq = 125
    SO=seh.create_sound(fs=100e3,freq=freq,duration=(simdur*0.8)/1000.0,dbspl=65.0)
    SP=seh.create_spk(SO,fs=100e3,N=N,cf=2.0*freq,seed=65562)
    S=np.array(SP['spikes']*1000.0)
    R32=sem.SE_BS(S,Simdur=simdur,dt=dt,G_EoH=0.0,
                  StochasticEoH=False,N=N,G_Dend=0.016)
    R1=sem.SE_BS(S,Simdur=simdur,dt=dt,G_EoH=0.0,
                 StochasticEoH=False,N=1,G_Dend=0.0005)
    ...
    plt.tight_layout()
    #plt.show()
    pp = PdfPages('./Figure1/Figure1_raw.pdf')
    pp.savefig(dpi=600)
    pp.close()
