...
def g_template(
    style="e", dur=2, dt=0.01, tau_rise=0.15, tau_decay=0.2, gmax=0.055, cutshort=False
):
    """
...
def stochastic_pulsetrain(
    N=16, distritype="normal", DistParam=1.0, IPI=10.0, dur=400.0, dt=0.01, debug=False
):
    ...
    sp_raw_t = (
        np.array(
            range(int(round(IPI / dt)), int(round(dur / dt)), int(round(IPI / dt)))
        )
        * dt
    )
    allspt = []
